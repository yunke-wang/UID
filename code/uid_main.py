...
def main(args):
    ...
    if method_type == "IL":
        ...
        elif args.il_method == "uid":
            discriminator_updater = UID(state_dim=state_dim, action_dim=action_dim, args=args)
        elif args.il_method == "uidwail":
            discriminator_updater = UIDWAIL(state_dim=state_dim, action_dim=action_dim, args=args)
        elif args.il_method == "pugail":
            discriminator_updater = PUGAIL(state_dim=state_dim, action_dim=action_dim, args=args)
    ...
    for total_step in tqdm(range(0, args.max_step + 1), dynamic_ncols=True):
                ...
                if method_type == "IL":
                    if 'rex' not in args.il_method:
                        for i_d in range(0, args.d_step):
                            ...
                            t0_d = time.time()
                            if args.il_method == 'uid' or 'pugail':
                                discriminator_updater.update_discriminator(batch=batch, index=index, policy=policy_updater, writer=writer, total_step=total_step, d=i_d)
                            elif args.il_method == 'uidwail':
                                discriminator_updater.update_discriminator(batch=batch, index=index, writer=writer, total_step=total_step)
                            else:
        ...
        if total_step % log_interval == 0:
            ...
            writer.add_scalar('Eval/Distance', log_test['distance'], total_step)
            writer.add_scalar('Eval/Reward', log_test['avg_reward'], total_step)
