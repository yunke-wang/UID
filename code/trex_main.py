...
class REX():
    ...
    def load_data(self):
        ...
        self.m_return_list = []
        index_start = 0
        self.expert_state_list, self.expert_action_list, self.expert_reward_list, \
                self.expert_mask_list, self.worker_id_list = [], [], [], [], []

        k_spec_list = self.noise_level_list
        traj_path = "../imitation_data/TRAJ_h5/%s" % (self.env_name)
        # self.traj_name = "traj_type%d_N%d" % (self.c_data, self.demo_file_size)
        ...
        for k in range(0, len(k_spec_list)):
            ...
            if args.time_index == True:
                ...
                traj_filename = traj_path + (
                                "/%s_TRAJ-N%d_ckpt%d" % (args.env_name, args.demo_file_size, k_spec))

            hf = h5py.File(traj_filename + ".h5", 'r')

            if args.fix_length == True:
                expert_mask = hf.get('expert_masks')[:1000]
                self.expert_mask_list += [expert_mask][:1000]
                self.expert_state_list += [hf.get('expert_states')[:1000]]
                self.expert_action_list += [hf.get('expert_actions')[:1000]]
                self.expert_reward_list += [hf.get('expert_rewards')[:1000]]
                reward_array = hf.get('expert_rewards')[:1000]
                step_num = expert_mask.shape[0]
...
class Drex(REX):
    ...
    def bc_train(self, args, expert_traj):
...
if __name__ == "__main__":
    ...
    if args.il_method == 'trex':
        ...
        trex.get_traj()
        trex.rex_train()
        torch.save(trex.reward.cpu().state_dict(), './reward_pu_model/trex/{}_type{}_reward.pth'.format(env_name, args.c_data))

    elif args.il_method == 'drex':
        drex = Drex(args, device, state_dim, action_dim, env, a_bound)
        drex.load_data()
        policy_net = drex.bc_train(args, None)

        drex.pre_build(args.drex_noise, policy_net)
        drex.get_traj()
        drex.rex_train()
        torch.save(drex.reward.cpu().state_dict(), './reward_pu_model/drex/{}_type{}_reward.pth'.format(env_name, args.c_data))
