    ...
    def load_data(self):
        ...
        for k in range(0, len(k_spec_list)):
            ...
            if args.fix_length == True:
                    ...
                    expert_mask_list += [expert_mask][:1000]
                    expert_state_list += [hf.get('expert_states')[:1000]]
                    expert_action_list += [hf.get('expert_actions')[:1000]]
                    expert_reward_list += [hf.get('expert_rewards')[:1000]]
...
if __name__ == "__main__":
    ...
    if args.il_method == 'trex':
        ...
        drex.get_traj()
        drex.rex_train()
        torch.save(drex.reward.cpu().state_dict(), './reward_model/drex/{}_type{}_reward.pth'.format(env_name, args.c_data))
